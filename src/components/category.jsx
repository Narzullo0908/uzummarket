...
export default function Category({ category }) {
  ...
  return (
    <div className="grid grid-cols-5 gap-x-5 gap-y-8 max-md:grid-cols-2 max-md:gap-x-2 mt-40">
      {data.map((item) => {
        if (item.id != detail) {
          return (
            <Link
            ...
            >
              <div className="relative hover:shadow-[5px_5px_2px_0px_#00000014] rounded-[10px] transition-all duration-300">
                <div className="w-full flex items-center rounded-[10px] justify-center h-100 object-cover bg-[#EFEFEF] dark:bg-[#2A2A2A] p-5 mb-3">
                  ...
                  <span className="absolute top-5 right-5 cursor-pointer dark:bg-black/40 bg-white rounded-4xl flex items-center justify-center p-1">
                    <CiHeart size={24}/>
                  </span>
